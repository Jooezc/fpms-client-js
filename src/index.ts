...
class FpmsConnector {
    ...
    public loginOrRegisterBySMS(
        ...
        const self = this;
        const cmdString = this.buildCommandString("player", "playerLoginOrRegisterWithSMS",
            registerObj,
            (result: any) => {
    ...
    public loginByPassword(
        ...
        const self = this;
        const cmdString = this.buildCommandString("player", "phoneNumberLoginWithPassword",
            registerObj,
            (result: any) => {
                if (result.status === 200)
                    self.isLogin = true;
    ...
    public loginByToken(
        ...
        const self = this;
        const cmdString = this.buildCommandString("player", "authenticate",
            tokenObj,
            (result: any) => {
                if (result.status === 200)
                    self.isLogin = true;
                else if (result.status === 493) {// token 过期
    ...
    private async send(message: string, losePacketHandler?: Function, retryCount: number = 0) {
        try {
            ...
            if (retryCount < 2) {
                ...
                if (error.code === 403) {
                    losePacketHandler?.({ status: 100001, errorMessage: "Service is for users residing in the Phillippines only." });
                } else {
    ...
    private sign(dataObj: any): any {
        if (dataObj && this.apiConfig) {
            if (dataObj.deviceType === undefined)
                dataObj.deviceType = this.apiConfig.DEVICETYPE;
